/**
 * AdvancedAuthCommunicationPanel Component
 * 
 * Handles NPHIES Communication workflow for Advanced Authorizations:
...
const AdvancedAuthCommunicationPanel = ({ 
  ...
  const loadData = async () => {
    ...
    try {
      const [requestsRes, commsRes] = await Promise.all([
        api.getAdvancedAuthCommunicationRequests(advAuthId),
        api.getAdvancedAuthCommunications(advAuthId)
      ]);
  ...
  const handlePreviewPollBundle = async () => {
    ...
    setError(null);
    try {
      const result = await api.previewAdvancedAuthPollBundle();
  ...
  const fetchPreviewFromBackend = async () => {
    try {
      const payloads = buildPayloadsForApi();
      const result = await api.previewAdvancedAuthCommunicationBundle(
        advAuthId,
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (communicationType === 'unsolicited') {
        result = await api.sendAdvancedAuthUnsolicitedCommunication(advAuthId, payloads);
      } else {
        result = await api.sendAdvancedAuthSolicitedCommunication(advAuthId, selectedRequestId, payloads);
      }
  ...
  // Advanced authorizations come from payer and may always need communication
  const canSendCommunication = true;
                            ...
                            <div key={pIdx}>
                              ...
                              {payload.content_type === 'attachment' && (
                                <div className="p-2 bg-blue-50 rounded text-sm text-gray-700 flex items-center justify-between">
                                  ...
                                  {payload.has_data && (
                                    <button
                                      onClick={() => api.downloadAdvancedAuthCommunicationRequestAttachment(advAuthId, request.id, payload.index)}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full mx-4 max-h-[90vh] flex flex-col">
            ...
            {pollMetadata && (
              <div className="p-4 bg-purple-50 border-b border-purple-200">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600">Advanced Auth ID:</span>
                    <span className="ml-2 font-medium text-gray-900">{pollMetadata.advAuthId || pollMetadata.priorAuthId}</span>
                  </div>
                  <div>
...
export default AdvancedAuthCommunicationPanel;
