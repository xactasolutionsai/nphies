...
export default function StandardApprovals() {
  ...
  const loadForms = async () => {
    try {
      setLoading(true);
      const response = await api.getStandardApprovals({ limit: 1000 });
      // Handle nested response structure
      const data = response?.data?.data || response?.data || response || [];
      setForms(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error('Error loading standard approvals:', error);
      setForms([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteForm = async (id) => {
    if (window.confirm('Are you sure you want to delete this form?')) {
      try {
        setLoading(true);
        await api.deleteStandardApproval(id);
      ...
      render: (row) => (
        <div className="flex gap-2">
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              navigate(`/standard-approvals/${row.id}`);
            }}
          >
          ...
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              navigate(`/standard-approvals/${row.id}/edit`);
            }}
          >
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900">
                  Approvals and Claims Request Form
              </h1>
              <p className="text-gray-600 mt-2 text-lg">Manage approval and claims request forms</p>
            ...
            <Button 
              onClick={() => navigate('/standard-approvals/new')} 
