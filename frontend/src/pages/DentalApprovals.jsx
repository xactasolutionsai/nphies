...
export default function DentalApprovals() {
  ...
  const loadForms = async () => {
    try {
      setLoading(true);
      const response = await api.getDentalApprovals({ limit: 1000 });
      // Handle nested response structure
      const data = response?.data?.data || response?.data || response || [];
      setForms(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error('Error loading dental approvals:', error);
      setForms([]);
    } finally {
      setLoading(false);
    }
      ...
      render: (row) => (
        <div className="flex gap-2">
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              navigate(`/dental-approvals/${row.id}`);
            }}
          >
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900">
                Dental Approvals and Claims
              </h1>
              <p className="text-gray-600 mt-2 text-lg">Manage dental approval and claims application forms</p>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Dental Forms List</CardTitle>
        </CardHeader>
        <CardContent>
          <DataTable
            data={forms}
