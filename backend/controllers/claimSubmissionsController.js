...
class ClaimSubmissionsController extends BaseController {
  ...
  async getByIdInternal(id) {
    ...
    const [itemsResult, supportingInfoResult, diagnosesResult, responsesResult, attachmentsResult] = await Promise.all([
      ...
      // bundle_json is required so the frontend can fall back to parsing
      // ClaimResponse.error[] when the structured `errors` column is empty
      // (e.g. for legacy poll rows persisted before extraction was wired up).
      query('SELECT id, claim_id, response_type, outcome, disposition, nphies_claim_id, has_errors, errors, bundle_json, is_nphies_generated, nphies_response_id, received_at FROM claim_submission_responses WHERE claim_id = $1 ORDER BY received_at DESC', [id]),
  ...
  async previewBundle(req, res) {
    try {
      ...
      const claim = {
        ...
        eligibility_response_id: formData.eligibility_response_id || null,
        eligibility_response_system: formData.eligibility_response_system || null,
        eligibility_ref: formData.eligibility_ref || null,
        encounter_identifier: formData.encounter_identifier || null,
